export default async function handler(req, res) {
  ...
  function transform(events) {
    return events.filter(ev => ev.competitions?.[0]?.competitors?.length >= 2).map(ev => {
      ...
      const timeET = utcDate.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "America/New_York" });
  ...
  async function fetchRange(range) {
    try {
      const r = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard?dates=${range}&limit=200`);
  ...
  try {
    ...
    let allSeason = [];
    try {
      const r2 = await fetch("https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard?year=2027&seasontype=2&limit=400");
      const d2 = await r2.json();
      allSeason = d2.events || [];
    } catch {}
    const seen = new Set();
    const allEvents = [...allMonthly, ...allSeason].filter(ev => { if (seen.has(ev.id)) return false; seen.add(ev.id); return true; });
    ...
    if (gwNums.length > 0) {
      ...
      if (offset > 0) fixtures = fixtures.map(f => f.gw != null ? { ...f, gw: f.gw - offset, round: `gw${f.gw - offset}` } : f);
    }
