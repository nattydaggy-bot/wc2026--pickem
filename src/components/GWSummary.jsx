...
export default function GWSummary({ fixtures, members, results, banker = {}, onClose }) {
  ...
  // Calculate GW results
  const gwResults = useMemo(() => {
    const gwFixtures = fixtures.filter(f => f.gw === gw);
    const gwMembers = {};
    const results_by_match = {};

    // Get match results for this GW
    gwFixtures.forEach(f => {
      const r = results[f.id];
      if (r?.completed) {
        results_by_match[f.id] = r;
      }
    });

    // Calculate each member's GW score
    Object.entries(members).forEach(([uname, data]) => {
      const { byGw, score } = calcMemberScoreWithBanker(
        data?.picks || {},
        data?.banker || {},
        fixtures.filter(f => f.gw === gw),
        results
      );
    ...
    // Sort by score
    const sorted = Object.entries(gwMembers)
      .sort((a, b) => b[1].score - a[1].score || b[1].correct - a[1].correct);
    ...
    return {
      winner,
      allScores: sorted,
      matchResults: results_by_match,
      totalMatches: gwFixtures.length,
      completedMatches: Object.keys(results_by_match).length,
      isComplete: gwFixtures.length > 0 && Object.keys(results_by_match).length === gwFixtures.length,
    };
  ...
  const downloadSummary = () => {
    ...
    // For now, copy to clipboard as text
    let text = `🏆 EPL Pick'em GW${gw} Summary\n`;
    text += `═`.repeat(40) + `\n\n`;
    
    if (gwResults.winner) {
      text += `🥇 Winner: ${gwResults.winner[1].teamName}\n`;
      text += `   Score: ${gwResults.winner[1].score} pts\n`;
      text += `   Correct: ${gwResults.winner[1].correct}/${gwResults.winner[1].total}\n\n`;
    }
    ...
    if (gwResults.allScores.length > 10) {
    ...
    navigator.clipboard?.writeText(text).then(() => {
      alert("✅ Summary copied to clipboard! Share it anywhere.");
    }).catch(() => {
      // Fallback
      ...
      <div ref={summaryRef} style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          <div>
            ...
            <div style={{ fontSize: "0.7rem", color: "#555" }}>
              {gwResults.completedMatches}/{gwResults.totalMatches} matches completed
              {gwResults.isComplete && " ✅ Complete!"}
            </div>
          </div>
          <button onClick={onClose} style={{
        ...
        {gwResults.winner && (
          <div style={{
            ...
            <div style={{ fontSize: "0.6rem", color: GOLD, letterSpacing: "1px", marginTop: "0.25rem" }}>
              GAMEWEEK WINNER
            </div>
            <div style={{ fontSize: "1.2rem", fontWeight: "bold", color: "#fff" }}>
              {gwResults.winner[1].teamName}
            </div>
            <div style={{ fontSize: "0.9rem", color: GOLD }}>
              {gwResults.winner[1].score} pts
            </div>
            <div style={{ fontSize: "0.65rem", color: "#555" }}>
              {gwResults.winner[1].correct}/{gwResults.winner[1].total} correct
            </div>
          </div>
        )}
        ...
        {/* Match Results */}
        <div style={{ marginBottom: "1rem" }}>
          <div style={{ fontSize: "0.65rem", color: "#555", marginBottom: "0.5rem", fontWeight: "bold" }}>
            📋 MATCH RESULTS
          </div>
          {Object.entries(gwResults.matchResults).map(([fid, r]) => {
            ...
            return (
              <div key={fid} style={{
                ...
                <span>{f.homeShort} vs {f.awayShort}</span>
          ...
          {gwResults.allScores.slice(0, 15).map(([uname, data], i) => {
            ...
            return (
              <div key={uname} style={{
                ...
                <span>
                  {medal || `${i+1}.`} {data.teamName}
                </span>
                <span style={{ fontWeight: i < 3 ? "bold" : "normal", color: i === 0 ? GOLD : "#aaa" }}>
                  {data.score} pts ({data.correct}/{data.total})
                </span>
              </div>
            );
          })}
          {gwResults.allScores.length > 15 && (
            <div style={{ textAlign: "center", fontSize: "0.6rem", color: "#444", padding: "0.5rem" }}>
              + {gwResults.allScores.length - 15} more players
            </div>
          )}
        </div>
        ...
        {/* Share hint */}
        <div style={{ textAlign: "center", fontSize: "0.5rem", color: "#333", marginTop: "0.75rem" }}>
          Paste the summary anywhere to share with your league!
        </div>
      </div>
    </div>
  );
