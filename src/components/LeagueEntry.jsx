...
const GREEN="#00ff85"; const PURPLE="#37003c"; const PURPLE2="#1a0025";
const inputStyle={
  width:"100%",padding:"0.7rem 0.85rem",borderRadius:8,
  border:"1px solid rgba(0,255,133,0.2)",background:"rgba(0,255,133,0.05)",
  color:"#fff",fontSize:"0.95rem",boxSizing:"border-box",outline:"none",
};
const label={display:"block",fontSize:"0.62rem",letterSpacing:"1.5px",color:"rgba(255,255,255,0.35)",marginBottom:5,marginTop:"0.9rem",fontWeight:"bold"};
      ...
      <div style={{width:"100%",maxWidth:440}}>
        ...
        <div style={{textAlign:"center",marginBottom:"2.25rem"}}>
          <div style={{background:PURPLE,borderRadius:16,padding:"1.25rem 1.5rem",marginBottom:"1rem",border:`2px solid ${GREEN}33`}}>
            ...
            <div style={{fontSize:"0.65rem",letterSpacing:"3px",color:GREEN,fontWeight:"bold",marginBottom:4}}>
              PICK'EM
            </div>
          ...
          <div style={{display:"flex",gap:6,justifyContent:"center",flexWrap:"wrap"}}>
            {["Home Win","Draw","Away Win"].map(l=>(
              <span key={l} style={{fontSize:"0.65rem",background:"rgba(0,255,133,0.08)",
                border:`1px solid ${GREEN}33`,borderRadius:20,padding:"2px 10px",color:GREEN}}>
                {l}
              </span>
            ))}
          </div>
        ...
        {mode==="home"&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <button onClick={()=>goto("create")} style={{padding:"0.95rem",borderRadius:10,border:"none",background:GREEN,color:"#000",fontWeight:"bold",fontSize:"1rem",cursor:"pointer"}}>
              🚀 Create a League
            </button>
            <div style={{textAlign:"center",fontSize:"0.72rem",color:"rgba(255,255,255,0.25)"}}>Have a code from a friend?</div>
            <button onClick={()=>goto("join")} style={{padding:"0.95rem",borderRadius:10,border:`1px solid ${GREEN}55`,background:"rgba(0,255,133,0.06)",color:GREEN,fontWeight:"bold",fontSize:"1rem",cursor:"pointer"}}>
              🔗 Join a League
            </button>
            <div style={{textAlign:"center",fontSize:"0.72rem",color:"rgba(255,255,255,0.25)"}}>Already joined?</div>
            <button onClick={()=>goto("login")} style={{padding:"0.95rem",borderRadius:10,border:"1px solid rgba(255,255,255,0.12)",background:"transparent",color:"rgba(255,255,255,0.6)",fontWeight:"bold",fontSize:"1rem",cursor:"pointer"}}>
              👤 Log In
            </button>
          </div>
        )}
          ...
          return(
            <div style={{background:"rgba(0,255,133,0.04)",border:`1px solid ${GREEN}22`,borderRadius:12,padding:"1.4rem"}}>
              ...
              <button onClick={submit} disabled={busy} style={{
                ...
                background:busy?"#333":cfg.color,color:cfg.textColor,
                fontWeight:"bold",fontSize:"0.95rem",cursor:busy?"not-allowed":"pointer",marginTop:"1.2rem",
              }}>
